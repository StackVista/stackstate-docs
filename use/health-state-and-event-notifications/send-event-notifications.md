...
3. Select the trigger event and event handler to run: 
   ...
   * **Run event handler** - the event handler function that will run whenever the selected event is generated. For health state changed events, StackState ships with event handler functions that can send an event notification via email, Slack or SMS, or POST to an HTTP webhook. You can also create your own custom functions. [Read more about event handler functions](/configure/topology/event-handlers.md#event-handler-functions).
...
StackState will group unhealthy components in a view into problems and generate events. Event handlers can listen to the events generated by problems to trigger event notifications and actions, however, this is not supported out of the box in StackState v4.3. To run an event handler in response to problem events in a view, you will need to [create a custom event handler](/configure/topology/event-handlers.md).
