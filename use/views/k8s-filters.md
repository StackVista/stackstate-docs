...
You can use the in-built [SUSE Observability Query Language \(STQL\)](../../develop/reference/k8sTs-stql_reference.md) to build an advanced topology filter that zooms in on a specific area of your topology.
...
The **View Filters** panel on the left of the SUSE Observability UI can be used to filter the events shown in the [Events Perspective](k8s-events-perspective.md). They're also included in the **Event** list in the right panel **View summary** tab and the details tabs - **Component details** and **Direct relation details**.
...
| **Source** | Events can be generated by SUSE Observability or retrieved from an external source system, such as Kubernetes or ServiceNow, by an integration. Click the **Source** filter box to open a list of all source systems for events that have been generated for the currently filtered components in the current time window. Select one or more source systems to see only those events.  |
