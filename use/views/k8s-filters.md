...
You can use the in-built [Rancher Observability Query Language \(STQL\)](../../develop/reference/k8sTs-stql_reference.md) to build an advanced topology filter that zooms in on a specific area of your topology.
...
The **View Filters** panel on the left of the Rancher Observability UI can be used to filter the events shown in the [Events Perspective](k8s-events-perspective.md). They're also included in the **Event** list in the right panel **View summary** tab and the details tabs - **Component details** and **Direct relation details**.
