...
2. [Write the topology query (STQL) to select the components](k8s-add-charts.md#write-the-topology-query)
3. [Write the PromQL query for the desired metric](k8s-add-charts.md#write-the-promql-query)
4. [Bind the correct time series to each component](k8s-add-charts.md#bind-the-correct-time-series-to-each-component)
...
Go to the [metric explorer](k8sTs-explore-metrics.md) of your StackState instance, http://your-stackstate-instance/#/metrics, and use it to query for the metric of interest. The explorer has auto-completion for metrics, labels, label values but also PromQL functions, and operators to help you out. Start with a short time range of, for example, an hour to get the best results.
