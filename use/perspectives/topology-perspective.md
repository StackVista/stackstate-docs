...
If a [basic filter](/use/perspectives/topology-perspective.md#filtering) or [advanced filter query](/configure/topology_selection_advanced/) exceeds the configured filtering limit, you will be presented with an error on screen and no topology visualization will be displayed.

Note that the filtering limit is applied to the total amount of elements that need to be loaded and not the amount of elements that will be displayed. For example:
```text
