...
Some advanced filtering options are compatible with basic filtering, but can't be set or adjusted as a basic filter. When these advanced filters are set in a way that's compatible with basic filtering, the box **Other filters** will be shown in the View Filters panel with details of the affected components:
