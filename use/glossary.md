...
* **Agent** - Software that runs on hosts. Collects events and metrics from hosts and sends them to Rancher Observability.
* **baseline** - Information used as a starting point by which to compare other information. In Rancher Observability it's used for comparison with the metric stream. It learns from the historical data and calculates the baseline on the current data.
* **check** - Defines the status of the component/relation and therefore represents the graphic presentation of the health status in the Rancher Observability view. It's responsible for determining the health status of the component based on the telemetry streams.
...
* **data source** - Defines how Rancher Observability recognizes components of specified type during the topology creation process.
...
* **event** - Rancher Observability records every change detected in the IT environment as an event. Events are always bound to one or more topology elements.
...
* **integration** - A link between an external data source and Rancher Observability as defined in a StackPack.
...
* **mapping** - In the synchronization process it specifies the transformation of external system topological data into Rancher Observability based on component/relation type.
...
* **monitor** - A declarative definition of a validation rule that uses available 4T data to compute health states and attach them to the Rancher Observability Topology.
* **monitor function** - Defines the underlying algorithm used to compute the health states produced by a monitor.
* **monitor runner** - A subsystem of Rancher Observability responsible for execution of the logic of a monitor in a timely manner.
...
* **STQL \(Rancher Observability Query Language\)** - A built-in query language used to create advanced filters. It's a typed language, which means that each supported type of data is predefined as a part of STQL.
* **STJ \(Rancher Observability Templated JSON\)** - A JSON file with Rancher Observability specific template placeholders.
* **STY \(Rancher Observability Templated YAML\)** - A YAML file with Rancher Observability specific template placeholders.
...
* **telemetry stream** - Component or relation specific data collected and sent over to Rancher Observability.
