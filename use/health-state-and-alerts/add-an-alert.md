...
In StackState, telemetry flows through components in the topology as either metric or event streams. These telemetry streams are used by [health checks](/use/health-state-and-alerts/create-a-health-check.md) to determine the health state of each component. Whenever there is a change to the health state or propagated health state of a component, an event is generated. If the health state change of a significant number of components in a view changes, this will also trigger the view health state to change and generate an event.
