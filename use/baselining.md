...
{% hint style="info" %}
This is good default choice for baselining. It works well for stationary as well as seasonal metrics.
{% endhint %}
...
This baseline functions works well for stationary metrics \(e.g. data center temperature, average response time, error count\). It uses the Exponential Weighted Moving Average \(EWMA\) algorithm. It is the same as the `Stationary Auto-Tuned Baseline`, but leaves the tuning up to you. This is not recommended.
...
To configure a check to alert on anomalies follow these steps:
...
Creating a baseline and creating a check for a baseline metric stream need not be a manual effort. To configure baselines automatically for new components you can simply update the corresponding [component/relation template](../concepts/component_and_relation_templates.md).
