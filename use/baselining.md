...
<<image of metric stream with baseline goes here>> 
...
To configure a basline for a metric stream go to the metric stream on a component or relation and select "add baseline" from the metric stream context menu (accessed through the triple dots next to the name of the metric stream). 
...
This is good default choice. It works well for stationary as well as seasonal metrics. 
