...
* The following changes to configuration files must be manually processed if you are using a customised or generated configuration:
  * `etc/stackstate-receiver/application.conf`:
    * New parameter `stackstate.baseUrl` must be added or StackState will fail to start up. This is the public URL for StackState and will be made available via the [UI script API](/develop/reference/scripting/script-apis/ui.md#function-baseurl).
