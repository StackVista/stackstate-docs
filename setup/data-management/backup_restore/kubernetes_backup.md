...
{% hint style="warning" %}
The S3 buckets used to store the backups can be encrypted with an Amazon S3-managed key (SSE-S3). Encryption with AWS KMS keys stored in AWS Key Management Service (SSE-KMS) is not supported and will result in error such as this one in the Elasticsearch logs:
```
