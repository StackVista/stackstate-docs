...
* Encryption with Amazon S3-managed keys (SSE-S3) is supported.
* Encryption with AWS KMS keys stored in AWS Key Management Service (SSE-KMS) is **not supported**. This will result in errors such as this one in the Elasticsearch logs:
