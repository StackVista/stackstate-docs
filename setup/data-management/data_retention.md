...
StackState imposes data retention limits to save storage space and improve performance. You can configure the data retention period balance between the amount of data stored, StackState performance, and data availability.

## Retention of topology graph data

By default, topology graph data will be retained for 8 days. StackState always retains the latest state of the topology graph and only removes history that is older than 8 days. You can check and alter the configured retention period this using the StackState CLI.
...
| `splittingStrategy` | `"days"` | The frequency of creating new indices. Can be one of "none", "hours", "days", "months" or "years". If "none" is specified, there will be only one index.                                                                                                    |
