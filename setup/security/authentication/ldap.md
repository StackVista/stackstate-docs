...
Rancher Observability can use an LDAP server \(including AD\) to authenticate against and to get roles/groups from. It does require a running LDAP server that's accessible to Rancher Observability.

The LDAP main directory and all subdirectories will be checked for user files. The bind credentials in the Rancher Observability configuration are used to authenticate Rancher Observability on the LDAP server. After authentication, Rancher Observability passes the top LDAP directory name for the user that wants to log in to Rancher Observability.

## Configure Rancher Observability for LDAP

### Kubernetes

To configure Rancher Observability to authenticate using an LDAP authentication server on Kubernetes, LDAP details and user role mapping needs to be added to the file `authentication.yaml`. For example:
...
stackstate:
  authentication:
    ...
    # standard subjects in Rancher Observability (guest, powerUser, admin and platformAdmin)
    roles:
...
Follow the steps below to configure Rancher Observability to authenticate using LDAP:
...
1. In `authentication.yaml` - add LDAP details \(see the example above\):
   ...
   * **bind** - Optional, used to authenticate Rancher Observability to LDAP server if the LDAP server doesn't support anonymous LDAP searches.
   * **userQuery parameters and groupQuery parameters** - The set of parameters inside correspond to the base dn of your LDAP where users and groups can be found. The first one is used for authenticating users in Rancher Observability, while the second is used for retrieving the group of that user to determine if the user is an Administrator, Power User or a Guest.
   ...
   * **emailKey** - The name of the attribute that's used as the email address in Rancher Observability.
...
2. In `authentication.yaml` - map user roles from LDAP to the correct Rancher Observability subjects \(see the example above\):
   * **roles** - for details, see the [default Rancher Observability roles](../rbac/rbac_permissions.md#predefined-roles). More Rancher Observability roles can also be created, see the [RBAC documentation](../rbac/).
3. Store the file `authentication.yaml` together with the `values.yaml` from the Rancher Observability installation instructions.
