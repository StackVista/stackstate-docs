...
Rancher Observability Agent v2.19.x is supported to monitor the following versions of Kubernetes or OpenShift:
...
* To also include the topology/telemetry payloads sent to Rancher Observability in the Agent log, set `--set-string 'global.extraEnv.open.STS_LOG_PAYLOADS'='true'`.
...
To uninstall the Rancher Observability Cluster Agent and the Rancher Observability Agent from your Kubernetes or OpenShift cluster, run a Helm uninstall:
...
* [Rancher Observability Agent Helm Chart documentation \(github.com\)](https://github.com/StackVista/helm-charts/tree/master/stable/stackstate-agent)
