...
The [sidecar injection mechanism](k8sTs-agent-request-tracing.md#enabling-the-trace-header-injection-sidecar), which gets enabled when using `--set httpHeaderInjectorWebhook.enabled=true` when installing the agent, creates a self-signed certificate and uses a `ClusterRole` which grants write access to `Secret` and `MutatingWebhookConfiguration` objects in the Kubernetes cluster.
...
1. Generate a self-signed certificate [locally](k8sTs-agent-request-tracing-certificates.md#generate-a-certificate-locally).
2. Use the k8s [cert-manager](https://cert-manager.io/) (if it already on the cluster) [with a `ClusterIssuer`](k8sTs-agent-request-tracing-certificates.md#generate-a-certificate-using-the-cert-manager).
