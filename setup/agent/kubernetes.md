...
In the example below, the StackState Agent images will be extracted from the archive, imported by Docker, and re-tagged to the registry given by the `-d` flag, in this example, `localhost`.  The `-t` argument (dry-run) is provided to show the work that will be performed:
