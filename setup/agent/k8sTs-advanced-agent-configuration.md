...
A number of advanced configuration options are available for Rancher Observability Agent V2. These can be set either in the `stackstate.yaml` configuration file \(Linux and Windows\) or using environment variables \(Docker, Kubernetes and OpenShift\).

## Reduce data production

The Rancher Observability Agent V2 collection interval can be configured. This will reduce the amount of data produced by the Agent.

{% tabs %}
{% tab title="Kubernetes, OpenShift" %}
To configure the collection interval of the Kubernetes and system level integrations, create a `values.yaml` file with the below contents and specify this when you install/upgrade Rancher Observability Agent V2. In this `values.yaml` example, the `min_collection_interval` has been set to double the default setting. This should result in a noticeable drop in the amount of data produced. If required, you can increase the interval further, however, the aim should be to find a balance between the frequency of data collection and the amount of data received by Rancher Observability: 
...
Specify the `values.yaml` file during installation / upgrade of the Rancher Observability Agent with the `--values` argument:
...
To reduce data production in Rancher Observability Agent running in a Docker container:
...
To reduce data production in Rancher Observability Agent running on Linux:
...
To reduce data production in Rancher Observability Agent running on Windows:

1. Configure the `min_collection_interval` for each of the following system integrations. The default setting is `15` seconds. Doubling this value should result in a noticeable drop in the amount of data produced. If required, you can increase the interval further, however, the aim should be to find a balance between the frequency of data collection and the amount of data received by Rancher Observability: 
   - **Memory** - `C:\ProgramData\Rancher Observability\conf.d\memory.d\conf.yaml`
   - **CPU** - `C:\ProgramData\Rancher Observability\conf.d\cpu.d\conf.yaml`
   - **Disk** - `C:\ProgramData\Rancher Observability\conf.d\disk.d\conf.yaml`
   - **Load** - `C:\ProgramData\Rancher Observability\conf.d\load.d\conf.yaml`
   - **File handle** - `C:\ProgramData\Rancher Observability\conf.d\file_handle.d\conf.yaml`
2. Set the intervals for process, container and connection gathering in `C:\ProgramData\Rancher Observability\stackstate.yaml`:
    ```yaml
    process_config:
      intervals:
        container: 40
...
Processes reported by Rancher Observability Agent V2 can optionally be filtered using a blacklist. Using this in conjunction with inclusion rules will allow otherwise excluded processes to be included.
...
To specify a blacklist or inclusions, set the associated environment variables and restart Rancher Observability Agent V2.
...
To specify a blacklist or inclusions, edit the below settings in the Agent configuration file `stackstate.yaml` and restart Rancher Observability Agent V2.

* **Linux** - `/etc/stackstate-agent/stackstate.yaml`
* **Windows** - `C:\ProgramData\Rancher Observability\stackstate.yaml`
...
The trace Agent will be enabled by default when Rancher Observability Agent is installed. It's required to receive traces in Rancher Observability. In case it has been disabled, you can enable it again using the instructions below.

{% tabs %}
{% tab title="Docker" %}
To enable tracing on Rancher Observability Agent running on Docker, edd the following parameters to your `docker run` command:
- `-e STS_APM_URL="https://stackstate-ip:receiver-port/stsAgent"`
  - The default Rancher Observability Receiver port is `7077`.
- `-e STS_APM_ENABLED="true"`
  - Allows the Rancher Observability Trace Agent to capture traces.
{% endtab %}
{% tab title="Linux" %}
To enable tracing on Rancher Observability Agent running on Linux, edit the configuration file `/etc/stackstate-agent/stackstate.yaml`and set the following variables:
...
{% endtab %}
{% tab title="Windows" %} 
To enable tracing on Rancher Observability Agent running on Windows, edit the configuration file `C:\ProgramData\Rancher Observability\stackstate.yaml` and change the following variables:
- `apm_sts_url="https://stackstate-ip:receiver-port/stsAgent"`
  - The default Rancher Observability Receiver port is `7077`.
- `enabled="true"`
  - Can be found under `apm_config.enabled`.
...
{% tabs %}
{% tab title="Docker, Kubernetes, OpenShift" %}
To disable a feature, set the associated environment variable and restart Rancher Observability Agent V2.
...
{% tab title="Linux, Windows" %}
To disable a feature, edit the below settings in the Agent configuration file `stackstate.yaml` and restart Rancher Observability Agent V2.
