...
{% hint style="info" %}
Note that the `values.yaml` example below includes configuration to [enable checksAgent](/setup/agent/kubernetes.md#enable-cluster-checks) and [run the Kubernetes_state check as a cluster check](/stackpacks/integrations/kubernetes.md#configure-cluster-check-kubernetes_state-check).
{% endhint %}
