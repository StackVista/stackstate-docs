...
* The required version of JDK has been updated - StackState now requires OpenJDK 11. This must be present on the installation machine before upgrading StackState. In case of having a mismatching JDK, StackState will fail to start. Note that if you are running StackState on CentOS or Red Hat, the previous version of JDK (JDK 8) should also be removed from the machine.
* All Splunk checks can now run on Agent V2. If you have any Splunk checks running on Agent V1 (legacy) (Splunk Metrics, Splunk Events or Splunk Topology V1), you should [upgrade to Agent V2 and migrate any checks configured to run on Agent V1 \(legacy\)](/setup/agent/migrate-agent-v1-to-v2/). Agent V1 (legacy) and will be deprecated in a future release of StackState.
