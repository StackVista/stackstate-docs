...
- The old `server` pod has been replaced by a number of separate pods. Any custom configuration in `components.server.config` should be moved to `components.api.config`.
