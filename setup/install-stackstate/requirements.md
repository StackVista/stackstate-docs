...
* **Amazon Elastic Kubernetes Service (EKS):** 1.20 to 1.22
* **Azure Kubernetes Service (AKS):** 1.20 to 1.23
* **OpenShift:** 4.8 to 4.10

### Node sizing

For a standard deployment, the StackState Helm chart will deploy back end services in a redundant setup with 3 instances of each service. See below for the number of nodes required for environments with out-of-the-box settings, note that these may change based on system tuning:
...
StackState has to be reachable on port 7070 by any supported browser. StackState port 7077 must be reachable from any system that's pushing data to StackState
