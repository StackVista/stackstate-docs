...
* Request access credentials to pull the SUSE Observability Docker images from [SUSE Observability support](https://support.stackstate.com/).
...
3. [Deploy SUSE Observability with Helm](kubernetes_install.md#deploy-stackstate-with-helm)
4. [Access the SUSE Observability UI](kubernetes_install.md#access-the-stackstate-ui)

### Create namespace

Start by creating the namespace where you want to install SUSE Observability and deploy the secret in that namespace. In our walkthrough we will use the namespace `stackstate`:
...
The SUSE Observability `values.yaml` file can be generated by running a separate Helm Chart, the `stackstate/stackstate-values` chart. A sample command line is:
...
| Base URL | `baseUrl` | The `<STACKSTATE_BASE_URL>`. The external URL for SUSE Observability that users and agents will use to connect. For example `https://stackstate.internal`. If you haven't decided on an Ingress configuration yet, use `http://localhost:8080`. This can be updated later in the generated file.                                                                                                    |
| Username and password\*\* | `-u` `-p` | The username and password used by SUSE Observability to pull images from quay.io/stackstate repositories. For air-gapped environments these need to be the username and password for the local docker registry. |
...
3. Deploy the latest SUSE Observability version to the `stackstate` namespace with the following command:
...
2. Deploy the latest SUSE Observability version to the `stackstate` namespace with the following command:
