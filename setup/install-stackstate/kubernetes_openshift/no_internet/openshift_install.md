...
{% hint style="info" %}
This is the guide for installing in an air-gapped environment, i.e. an environment with no direct internet access from the cluster and the systems from which StackState is installed onto the cluster. Use the [normal installation guide](../openshift_install.md) to install on a cluster that does have internet access.
{% endhint %}


1. [Download the pre-requisites](./download_prerequisites.md)
2. [Create the project where StackState will be installed](openshift_install.md#create-project)
3. [Generate the `values.yaml` file](openshift_install.md#generate-values.yaml)
4. [Create the `openshift-values.yaml` file](openshift_install.md#create-openshift-values.yaml)
5. [Automatically install the Cluster Agent for OpenShift](openshift_install.md#automatically-install-the-cluster-agent-for-openshift)
6. [Deploy StackState with Helm](openshift_install.md#deploy-stackstate-with-helm)
7. [Access the StackState UI](openshift_install.md#access-the-stackstate-ui)
8. [Manually create `SecurityContextConfiguration` objects](openshift_install.md#manually-create-securitycontextconfiguration-objects)
