...
1. [Deploy SUSE Observability with Helm](openshift_install.md#deploy-stackstate-with-helm)
1. [Access the SUSE Observability UI](openshift_install.md#access-the-stackstate-ui)
...
Start by creating the project where you want to install SUSE Observability. In our walkthrough we will use the namespace `stackstate`:
...
1. Before you deploy:
   ...
   * [Create `openshift-values.yaml`](#create-openshift-values.yaml)
   * [Create `nonha_values.yaml`](/setup/install-stackstate/kubernetes_openshift/non_high_availability_setup.md)
5. Deploy the latest SUSE Observability version to the `stackstate` namespace with the following command:
