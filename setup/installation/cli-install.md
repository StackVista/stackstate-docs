...
{% hint style="warning" %}
The configuration generated by the wizard will only work when StackState has been configured to use either LDAP authentication or the internal authentication. When using OIDC or KeyCloak authentication, please follow the directions in the [manual configuration section](cli-install.md#manual-configuration).
{% endhint %}
