...
1. [Create the project where StackState will be installed](install_stackstate.md#create-project)
...
Furthermore, the StackState installation needs to add two specific OpenShift `SecurityContextConfiguration` objects to the OpenShift installation. These are needed for:

- The Agent that runs the Kubernetes checks: This process needs to run a privileged container with direct access to the host(network) and volumes.
- The HBase HDFS namenodes: These processes need to be able to run a `chmod` on their volumes and run with a specific (pre-known) UID, in order for the clients to write to the store.

The values that are needed for a successful OpenShift deployment are:
...
- `<RANDOM_TOKEN>`: A 32 character random token. This can be generated by executing `head -c32 < /dev/urandom | $md5sum_command  | cut -c-32`
