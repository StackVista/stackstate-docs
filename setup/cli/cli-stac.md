...
StackState receives topology, telemetry and trace data via the Receiver API. If you want to push information to StackState using the `stac` CLI, you will need to provide a Receiver API key. This is the same API key that's used by the StackState Agent and is available from your administrator.
