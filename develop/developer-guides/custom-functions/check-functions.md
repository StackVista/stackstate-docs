...
Check functions can be written to listen to anomaly events generated by the StackState Autonomous Anomaly Detector. For example, to trigger a `DEVIATING` health status if an anomaly with certain parameters is found for a specified metric stream.
