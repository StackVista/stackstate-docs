...
Check functions can be written to listen to anomaly events generated by the StackState Autonomous Anomaly Detector. For example, a `DEVIATING` health status can then be triggered if an anomaly with certain parameters is found for a specified metric stream.

### Parameters

The parameters required for a check function to listen to anomaly events are:
