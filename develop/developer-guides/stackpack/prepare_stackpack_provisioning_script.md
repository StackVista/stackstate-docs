...
* `preInstall` - run when installing the very first instance of a StackPack - it's meant to install all the objects that each of the instances will share.
* `install` - run for every installed instance of a StackPack - it's meant to install instance-specific objects, that will be of use for this instance only.
* `waitingForData` - allows the StackPack creator to check whether any external service that this StackPack communicates with is properly sending data that this StackPack can process. By default, it just transitions to the `INSTALLED` state.
* `upgrade` - run for every installed instance of a StackPack when the user upgrades their StackPack version.
* `uninstall` - run for every instance when it's being uninstalled - it's meant to clean up all the instance-specific objects.
* `postUninstall` - run when uninstalling the very last instance of a StackPack - it's meant to clean up all the StackPack-shared objects.
...
ProvisioningIO<scala.Unit> install(Map<String, Object> config) {
    ...
    return context().sts().importSnapshot(context().stackPack().namespacePrefix(), "templates/{stackpack}.sty", templateArguments)
}
