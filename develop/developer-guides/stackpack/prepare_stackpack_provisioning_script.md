...
* `intakeApi()` - returns an object representing the StackState Receiver API that receives incoming data. The object supplies the following functions:
  * `apiKey()` - returns the `API_KEY`, this is the API key for the StackState Receiver API. Also referred to as the `<STACKSTATE_RECEIVER_API_KEY>`.
  * `baseUrl()` - returns the `RECEIVER_BASE_URL`, this is the base URL for the StackState Receiver API.
