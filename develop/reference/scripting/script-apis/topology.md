...
* `at(time: Instant or currentTimeslice)` - specifes a [time](/develop/reference/scripting/script-apis/time.md) for which the query should be executed. Use an `instant` to query for transactions that started at a specific at any point in the past. Use `currentTimeslice` to query for all transactions currently started or in progress.
