...
{% hint style="info" %}
This page describes the Splunk Metrics check running on Rancher Observability Agent V2. 
...
When the [Splunk StackPack](splunk_stackpack.md) is installed in Rancher Observability, you can configure the Splunk Metrics check on Rancher Observability Agent V2 to begin collecting Splunk metrics data.

Metrics are collected from Splunk by executing Splunk saved searches that have been specified in the Rancher Observability Agent V2 Splunk Metrics check configuration. In order to receive Splunk metrics data in Rancher Observability, you will therefore need to add configuration to both Splunk and Rancher Observability Agent V2.
...
* [Rancher Observability Agent V2 Splunk Metrics check](splunk_metrics.md#agent-check) - a Splunk Metrics check should be configured to connect to your Splunk instance and execute the relevant Splunk saved searches.

The Splunk Metrics check on Rancher Observability Agent V2 will execute all configured Splunk saved searches periodically. Data will be requested from the last received metric timestamp up until now.
...
Rancher Observability Agent V2 executes the Splunk saved searches configured in the [Splunk Metrics Agent check configuration file](splunk_metrics.md#agent-check) and pushes retrieved data to Rancher Observability as a telemetry stream. The following fields from the results of a saved search are sent to Rancher Observability:
...
The example Splunk saved search above would result in the following metric data in Rancher Observability:
...
To enable the Splunk Metrics integration and begin collecting metrics data from your Splunk instance, the Splunk Metrics check must be configured on Rancher Observability Agent V2. The check configuration provides all details required for the Agent to connect to your Splunk instance and execute a Splunk saved search.
...
1. Edit the Rancher Observability Agent V2 configuration file `/etc/stackstate-agent/conf.d/splunk_metric.d/conf.yaml`.
2. Under **instances**, add details of your Splunk instance:
   ...
   * **tags** - Optional. Can be used to apply specific tags to all reported metrics in Rancher Observability.
...
8. Metrics retrieved from splunk are available in Rancher Observability as a metrics telemetry stream in the `stackstate-metrics` data source. This can be [mapped to associated components](../../../use/metrics/add-telemetry-to-element.md).
...
2. Restart the Rancher Observability Agent to apply the configuration changes.

## Splunk metrics in Rancher Observability

Metrics retrieved from splunk are available in Rancher Observability as a metrics telemetry stream in the `stackstate-metrics` data source. This can be [mapped to associated components](../../../use/metrics/add-telemetry-to-element.md).
