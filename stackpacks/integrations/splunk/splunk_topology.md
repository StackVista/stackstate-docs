...
{% hint style="info" %}
This page describes the Splunk Topology check running on Rancher Observability Agent V2. 
...
When the [Splunk StackPack](splunk_stackpack.md) is installed in Rancher Observability, you can configure the Splunk Topology check on Rancher Observability Agent V2 to begin collecting Splunk topology data.

The Rancher Observability Splunk Topology integration collects topology from Splunk by executing Splunk saved searches from [Rancher Observability Agent V2](../../../setup/agent/about-stackstate-agent.md). In order to receive Splunk topology data in Rancher Observability, configuration needs to be added to both Splunk and Rancher Observability Agent V2:
...
* [Rancher Observability Agent V2 Splunk Topology check](splunk_topology.md#agent-check) - a Splunk Topology check should be configured to connect to your Splunk instance and execute the relevant Splunk saved searches.

The Splunk Topology check on Rancher Observability Agent V2 will execute all configured Splunk saved searches periodically to retrieve a snapshot of the topology at the current time.
...
In the Splunk Topology integration, Rancher Observability Agent V2 executes the Splunk saved searches configured in the [Splunk Topology Agent check](splunk_topology.md#agent-check) and pushes retrieved data to Rancher Observability as components and relations. The fields from the results of a saved search that are sent to Rancher Observability are described below.

### Topology components

The following fields from the results of a saved search are sent to Rancher Observability for topology components:
...
| All other fields     | [Splunk default fields \(docs.splunk.com\)](https://docs.splunk.com/Documentation/Splunk/9.0.1/Data/Aboutdefaultfields) other than `_time` will be filtered out of the result. Any other fields present in the result will be available in Rancher Observability in the `data` field of the properties `source` tab for a component. |
...
The example Splunk saved search above would result in the following topology component data in Rancher Observability:
...
The following fields from the results of a saved search are sent to Rancher Observability for topology relations:
...
The example Splunk saved search above would result in the following topology relation data in Rancher Observability:
...
To enable the Splunk Topology integration and begin collecting component and relation data from your Splunk instance, the Splunk Topology check must be configured on Rancher Observability Agent V2. The check configuration provides all details required for the Agent to connect to your Splunk instance and execute a Splunk saved search.
...
1. Edit the Rancher Observability Agent V2 check configuration file: `/etc/stackstate-agent/conf.d/splunk_topology.d/conf.yaml`
2. Under **instances**, add details of your Splunk instance:
   ...
   * **tags** - Optional. Can be used to apply specific tags to all reported topology in Rancher Observability.
...
7. Restart Rancher Observability Agent V2 to apply the configuration changes.
8. Once the Agent has restarted, wait for the Agent to collect data and send it to Rancher Observability.
