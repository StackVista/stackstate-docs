...
* Rancher Observability Agent V2 collects all service responses from the target AWS account.
...
* Logs are retrieved once a minute from CloudWatch and a central S3 bucket. These are mapped to associated components in Rancher Observability.
* Metrics are retrieved on-demand by the Rancher Observability CloudWatch plugin. These are mapped to associated components in Rancher Observability.
* [VPC FlowLogs](#configure-vpc-flowlogs) are retrieved once a minute from the configured S3 bucket. Private network traffic inside VPCs is analysed to create relations between EC2 and RDS database components in Rancher Observability.
...
AWS is a [Rancher Observability core integration](/stackpacks/integrations/about_integrations.md#stackstate-core-integrations "Rancher Observability Self-Hosted only").
...
* [Rancher Observability Agent V2](../../../setup/agent/about-stackstate-agent.md) installed on a machine which can connect to both AWS and Rancher Observability.
...
* The following AWS accounts:
  ...
  * An AWS account for Rancher Observability and the Rancher Observability Agent to use when retrieving data from the target AWS account(s). It's recommended to use a separate shared account for this and not use any of the accounts that will be monitored by Rancher Observability, but this isn't required. 
  * A user or role with a policy attached with the action to allow assuming the role stsIntegrationRole in the account that will be monitored. For details see the Rancher Observability docs on the required AWS policy.

### AWS accounts

It's recommended to have two different AWS accounts: One that's being monitored and another used for data collection.

* **Monitor Account** - used to [deploy a CloudFormation Stack](#deploy-the-aws-cloudformation-stack). The CloudFormation stack will create an IAM role that has the permissions required to retrieve data from this Monitor Account (`Rancher ObservabilityAwsIntegrationRole`). 
* **Data Collection Account** - used by Rancher Observability and Rancher Observability Agent to retrieve data from the Monitor Account. Requires an AWS policy granting permissions to assume the role `Rancher ObservabilityAwsIntegrationRole` in the Monitor Account. 

#### AWS policy

The policy below grants permission to assume the role `Rancher ObservabilityAwsIntegrationRole` that's created in each target AWS Monitor Account when the CloudFormation Stack is deployed. Rancher Observability and Rancher Observability Agent require this policy to collect data from the AWS Monitor Account. The policy should be created and attached to the AWS user(s) or IAM role(s) that will be used by Rancher Observability and the Rancher Observability Agent:
...
The policy can be made available to Rancher Observability and the Rancher Observability Agent in one of the following ways:

* **If Rancher Observability or Rancher Observability Agent run on EC2 or EKS AND the Data Collection Account and Monitor Account are in the same AWS organization**: 
  * [Attach an IAM role to the EC2 instance or EKS pod](#iam-role-on-ec2-or-eks).
* **In all other situations**: 
  * Rancher Observability: Attach the policy to the user [configured when the AWS StackPack instance is installed](#install-the-aws-stackpack).
  * Rancher Observability Agent: Attach the policy to the user [configured in the Agent AWS check](#configure-the-aws-check).

#### IAM role on EC2 or EKS

Rancher Observability Agent collects topology, logs and (if configured) VPC flow logs, and Rancher Observability pulls CloudWatch metrics from AWS. If Rancher Observability Agent or Rancher Observability run in an AWS environment and the Data Collection Account and Monitor Account are in the same AWS organization, an IAM role can be attached to the EC2 instance or EKS pod that they run on and used for authentication. This removes the need to specify an AWS Access Key ID and Secret when a StackPack instance is installed or in the Agent AWS check configuration.
...
2. Attach the created policy to the relevant IAM role:
   * [Rancher Observability/Agent running on EC2](aws-sts-ec2.md)
   * [Rancher Observability/Agent running on EKS](aws-sts-eks.md)
3. **To use the IAM role for Rancher Observability (CloudWatch metrics)**: When you install an AWS StackPack instance, set the following parameter values:
   * **AWS Access Key ID**: `use-role` 
   * **AWS Secret Access Key**: `use-role` 
4. **To use the IAM role for Rancher Observability Agent**: 
...
The Rancher Observability AWS CloudFormation Stack should be deployed in each AWS account that you will monitor. It provides the minimum level of access required for Rancher Observability and the Rancher Observability Agent to collect topology, telemetry and logs.

* [Quick deployment](aws.md#quick-deployment) - Deploy all resources to a region in an account using a link.
* [Rancher Observability CloudFormation template](aws.md#stackstate-template-deployment) - Download the Rancher Observability CloudFormation template to integrate into your own deployment workflow.
...
**It's recommended to use the** [**Rancher Observability CloudFormation template**](aws.md#stackstate-template-deployment) **wherever possible** as this provides an easy upgrade path for future versions and reduces the maintenance burden.
{% endhint %}
...
The table below includes links to deploy the template in popular AWS regions. For any regions not listed, follow the steps described for the [Rancher Observability template deployment](aws.md#stackstate-template-deployment).
...
#### Rancher Observability template deployment

The default Rancher Observability CloudFormation template can be used to deploy all necessary resources. It can be deployed to multiple AWS accounts and regions at once by deploying it in a CloudFormation StackSet. It's recommended to use this template as it provides an easy upgrade path for future versions and reduces the maintenance burden compared to creating a custom template.

* [Download the default Rancher Observability CloudFormation template \(stackstate-integrations-resources-eu-west-1.s3.eu-west-1.amazonaws.com\)](https://stackstate-integrations-resources-eu-west-1.s3.eu-west-1.amazonaws.com/aws-topology/cloudformation/stackstate-resources-1.3.cfn.yaml)
...
* **ExternalId** - A shared secret that the Rancher Observability Agent will present when assuming a role. Use the same value across all AWS accounts that the Agent is monitoring. Example: `uniquesecret!1`.
...
Install the AWS StackPack from the Rancher Observability UI **StackPacks** &gt; **Integrations** screen. You will need to enter the following details, these will be used to configure the StackPack instance within Rancher Observability and for Rancher Observability to query live telemetry from the AWS account. To create topology in Rancher Observability, you will also need to configure the AWS check on Rancher Observability Agent V2.

* **Role ARN** - the ARN of the IAM Role created by the cloudFormation stack. For example, `arn:aws:iam::<account id>:role/Rancher ObservabilityAwsIntegrationRole` where `<account id>` is the 12-digit AWS account ID being monitored. 
...
* **AWS Access Key ID** - The Access Key ID of the IAM user that will be used by Rancher Observability to collect CloudWatch metrics. This is the same as the IAM user used by the Agent to collect topology data and logs from AWS. If Rancher Observability is running within AWS, it may also be possible to [authenticate with an IAM role](#iam-role-on-ec2-or-eks).
* **AWS Secret Access Key** - The Secret Access Key of the IAM user that will be used by Rancher Observability to collect CloudWatch metrics. This is the same as the IAM user used by the Agent to collect topology data and logs from AWS. If Rancher Observability is running within AWS, it may also be possible to [authenticate with an IAM role](#iam-role-on-ec2-or-eks).

### Configure the AWS check

To enable the AWS check and begin collecting topology and log data from AWS, add the configuration below to Rancher Observability Agent V2.

{% tabs %}
{% tab title="Agent on Kubernetes" %}

If Rancher Observability Agent is running on Kubernetes, the AWS check should be configured as a [cluster check](/setup/agent/kubernetes-openshift.md#enable-cluster-checks).
...
2. Update the `values.yaml` file used to deploy the `stackstate-agent` with details of your AWS instance:
    ...
    - **role_arn** - In the example `arn:aws:iam::123456789012:role/Rancher ObservabilityAwsIntegrationRole`, substitute 123456789012 with the target AWS account ID to read.
...
If Rancher Observability Agent is running on a Linux VM:
...
2. You can also add optional configuration and filters: 
    ...
    - **tags** - Optional. Can be used to apply specific tags to all reported data in Rancher Observability.

3. [Restart the Rancher Observability Agent](/setup/agent/about-stackstate-agent.md#deployment) to apply the configuration changes.
4. Once the Agent has restarted, wait for data to be collected from AWS and sent to Rancher Observability.
{% endtab %}
{% endtabs %}
...
Rancher Observability and the Rancher Observability Agent require access to the internet to call the AWS APIs. If direct internet access can't be given, an HTTP proxy can be used to proxy the API calls. 

In the Rancher Observability AWS integration, CloudWatch metrics are pulled directly by Rancher Observability, while events and topology data are collected by the Rancher Observability Agent. This means that proxy details must be configured in two places to handle all requests - Rancher Observability for CloudWatch metrics and Rancher Observability Agent for topology and events data.

* **To configure a proxy for events and topology data collected by Rancher Observability Agent**, see how to [use an HTTP/HTTPS proxy for Rancher Observability Agent](/setup/agent/agent-proxy.md).
* **To configure a proxy for CloudWatch metrics collected by Rancher Observability**, follow the steps below:
  1. In the Rancher Observability UI, go to **Settings** > **Telemetry Sources** > **CloudWatch sources**.
...
AWS events are primarily used to provide real-time updates to topology. These events aren't displayed as Rancher Observability events.
...
The AWS service data shown below is available in Rancher Observability as components with the associated relations. The retrieved topology can be further enhanced by enabling [OpenTelemetry traces](/stackpacks/integrations/opentelemetry/opentelemetry-nodejs.md).
...
A high-level of overview of all resources necessary to run the Rancher Observability Agent with full capabilities is provided in the graph below. Users with intermediate to high level AWS skills can use these details to set up the Rancher Observability Agent resources manually. For the majority of installations, this isn't the recommended approach. Use the provided [Rancher Observability CloudFormation template](aws.md#stackstate-template-deployment) unless there are environment-specific issues that must be worked around.

![Account components](../../../.gitbook/assets/stackpack-aws-v2-account-components.svg)

Hourly and event-based updates collect data:

* Hourly full topology updates - collected by the Rancher Observability Agent using an IAM role with access to the AWS services.
* Event-based updates for single components and relations - captured using AWS services and placed into an S3 bucket for the Rancher Observability Agent to read.

If the Rancher Observability Agent doesn't have permission to access a certain component, it will skip it.

#### Rancher Observability Agent IAM Role

The bare minimum necessary to run the Rancher Observability Agent is an IAM role with necessary permissions. The Agent will always attempt to fetch as much data as possible for the supported resources. If a permission is omitted, the Agent will attempt to create a component with the data it has.
...
The AWS StackPack CloudFormation template with all resources necessary to run the AWS check on the Rancher Observability Agent. The installed resources are kept as minimal as possible. All costs incurred are minimal but variable, with costs scaling depending on how many events are emitted in a given account. In practice, the costs created by the AWS integration will be negligible.
...
#### Tags added as labels in Rancher Observability

The following labels will be added to imported AWS topology in Rancher Observability:
...
Note that topology with the label `stackpack:aws` was imported by the [AWS \(legacy\) integration](/stackpacks/integrations/aws/aws-legacy.md "Rancher Observability Self-Hosted only").

{% endhint %}

#### AWS tags used to build Rancher Observability topology

The special tags listed below can be added in AWS to influence how the imported topology is built in Rancher Observability:

* `stackstate-identifier` - The specified value will be added as an identifier to the Rancher Observability component.
* `stackstate-environment` - The Rancher Observability component will be placed in the specified environment.

## Troubleshooting

Check the Rancher Observability support site for:

* [The AWS StackPack troubleshooting guide](https://support.stackstate.com/hc/en-us/articles/4402736972050-Troubleshooting-Rancher Observability-AWS-StackPack).
...
To clean up the remaining resources inside your AWS account, remove any configured VPC flow logs and delete the Rancher Observability AWS Cloudformation stack from the AWS account being monitored. This can be done using the [AWS web console](aws.md#aws-web-console) or the [AWS CLI](aws.md#aws-cli).

#### AWS Web console

To delete the Rancher Observability AWS Cloudformation stack from an AWS account using the AWS web console:
...
5. Delete the CloudFormation template:
   ...
   2. Select the Rancher Observability CloudFormation template. This will be named `stackstate-resources` if created via the quick deploy method, otherwise the name was user-defined.
...
To delete the Rancher Observability AWS Cloudformation stack from an AWS account using the AWS CLI:

1. Set the region to remove Rancher Observability resources from:
...
* [Rancher Observability AWS \(legacy\) integration](aws-legacy.md "Rancher Observability Self-Hosted only")
