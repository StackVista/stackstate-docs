...
By default, installer tries to configure the package update channel, which would allow to update packages using the host package manager.
If you for any reason do not want this behavior, please include `STS_INSTALL_NO_REPO=yes` as an environment parameter:
...
On your host, download a copy of the PowerShell script from <https://stackstate-agent-2.s3.amazonaws.com/install.ps1> alongside with the agent installer in the `.msi` form.
