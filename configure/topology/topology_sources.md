...
| **Instance type** | The integration type. Select from the dropdown list. This list is populated with the type passed in the `instance` / `type` field in the [source JSON data](/configure/topology/topology_synchronization.md#topology-json-format). | 
| **Topic** | The Kafka topic to retrieve data from. Select from the dropdown listThis list is populated based on the `type` and `url` passed in the `instance` field in the [source topology JSON](/configure/topology/topology_synchronization.md#topology-json-format). | 
