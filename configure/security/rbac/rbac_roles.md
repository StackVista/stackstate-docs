...
2. Add new users and subjects to the logins table in the `application_stackstate.conf` as shown in the example below. Note that the default roles are always available (`stackstate-admin`, `stackstate-power-user` and `stackstate-guest`)
