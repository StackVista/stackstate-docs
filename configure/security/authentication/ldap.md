...
1. In `authentication.yaml` - add LDAP details \(see the example above\):
   ...
   * **groupMemberKey** - The name of the attribute that indicates whether a user is a member of a group. The constructed LDAP filter follows this pattern: `<groupMemberKey>=<user.dn>,ou=groups,dc=acme,dc=com`. To return all nested groups, set `gruoupMemberKey: "member:1.2.840.113556.1.4.1941:"`.
