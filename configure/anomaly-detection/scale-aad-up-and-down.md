...
* A configurable number of worker pods that run model selection, training and (near) real-time anomaly detection. Workers fetch their data from Rancher Observability and report back any found anomalies (or their absence).
...
For most cases, this snippet should be added to the `values.yaml` file used to deploy [Rancher Observability](/setup/install-stackstate/kubernetes_openshift/kubernetes_install.md).

## Evaluate use of resources

One of the most important uses of anomalies in the Rancher Observability product is in [anomaly health checks](../../use/checks-and-monitors/anomaly-health-checks.md). The following metrics can be used to determine if the AAD is putting the available resources to good use:
...
1. In the Rancher Observability UI, open the main menu on the left and go to the [Analytics environment](/use/stackstate-ui/analytics.md).
